...
!function (a, b) {
    ...
    b.factory("ngTableParams", ["$q", "$log", function (b, c) {
        var d = function (a) {
            ...
            this.data = [], this.parameters = function (b, e) {
                ...
                return a.isDefined(b) ? (a.isArray(b.data) && (b.total = b.data.length), i = a.extend(i, b), c.debug && c.debug("ngTable: set settings", i), this) : i
            }, this.page = function (b) {
                return a.isDefined(b) ? this.parameters({page: b}) : h.page
            }, this.total = function (b) {
                return a.isDefined(b) ? this.settings({total: b}) : i.total
                ...
                f.promise.then(function (b) {
                    ...
                    a.forEach(b, function (b) {
                        var c = a.isFunction(e) ? e(b) : b[e];
                        f[c] = f[c] || {data: []}, f[c].value = c, f[c].data.push(b)
                    });
                    var g = [];
                    for (var h in f)g.push(f[h]);
                    c.debug && c.debug("ngTable: refresh groups", g), d.resolve(g)
                }), this.getData(f, g)
            }, this.generatePagesArray = function (a, b, c) {
                var d, e, f, g, h, i;
                if (d = 11, i = [], h = Math.ceil(b / c), h > 1) {
                    i.push({type: "prev", number: Math.max(1, a - 1), active: a > 1}), i.push({type: "first", number: 1, active: a > 1}), f = Math.round((d - 5) / 2), g = Math.max(2, a - f), e = Math.min(h - 1, a + 2 * f - (a - g)), g = Math.max(2, g - (2 * f - (e - g)));
                    for (var j = g; e >= j;)i.push(j === g && 2 !== j || j === e && j !== h - 1 ? {type: "more", active: !1} : {type: "page", number: j, active: a !== j}), j++;
                    i.push({type: "last", number: h, active: a !== h}), i.push({type: "next", number: Math.min(h, a + 1), active: h > a})
                }
                return i
            }, this.url = function (b) {
                b = b || !1;
                ...
                i.$loading = !0, i.groupBy ? i.getGroups(a, i.groupBy, this) : i.getData(a, this), c.debug && c.debug("ngTable: reload data"), a.promise.then(function (a) {
                    i.$loading = !1, c.debug && c.debug("ngTable: current scope", i.$scope), d.data = i.groupBy ? i.$scope.$groups = a : i.$scope.$data = a, i.$scope.pages = d.generatePagesArray(d.page(), d.total(), d.count())
                })
            }, this.reloadPages = function () {
                var a = this;
                i.$scope.pages = a.generatePagesArray(a.page(), a.total(), a.count())
            };
            var h = this.$params = {page: 1, count: 1, filter: {}, sorting: {}, group: {}, groupBy: null}, i = {$scope: null, $loading: !1, data: null, total: 0, counts: [10, 25, 50, 100], getGroups: this.getGroups, getData: this.getData};
    ...
    var c = ["$scope", "ngTableParams", "$q", function (a, b) {
        ...
        }, !0), a.sortBy = function (b, c) {
            var d = a.parse(b.sortable);
            if (d) {
                var e = a.params.sorting() && a.params.sorting()[d] && "desc" === a.params.sorting()[d], f = c.ctrlKey ? a.params.sorting() : {};
                f[d] = e ? "asc" : "desc", a.params.parameters({sorting: f})
            }
        }
    }];
